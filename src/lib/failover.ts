  ...
  if (!keysArray || keysArray.length === 0) {
    return {
      ...
      logs: [{
        ...
        status: 'failed',
        errorDetail: 'No API keys stored in system key list.',
  ...
  for (let i = 0; i < keysArray.length; i++) {
    ...
    try {
      ...
      if (res.status === 429 || res.status === 401 || res.status === 403) {
        ...
        console.warn(`[Failover] Key ${i} (${masked}) failed with status ${res.status}. Rotating...`);
        continue;
      }
      ...
      if (!res.ok) {
        ...
        console.warn(`[Failover] Key ${i} failed. Status: ${res.status}`);
        continue;
      }
      ...
      console.warn(`[Failover] Key ${i} threw exception: ${err.message}. Rotating...`);
    }
  }
